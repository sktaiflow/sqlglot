...
def _derived_table_values_to_unnest(
    self: NewBigQuery.Generator, expression: exp.Values
) -> str:
...
def _returnsproperty_sql(
    self: NewBigQuery.Generator, expression: exp.ReturnsProperty
) -> str:
...
def _create_sql(self: NewBigQuery.Generator, expression: exp.Create) -> str:
...
def _array_contains_sql(self: NewBigQuery.Generator, expression: exp.ArrayContains) -> str:
    return self.sql(
        exp.Exists(
            this=exp.select("1")
            .from_(
...
def _ts_or_ds_add_sql(self: NewBigQuery.Generator, expression: exp.TsOrDsAdd) -> str:
    return date_add_interval_sql("DATE", "ADD")(self, ts_or_ds_add_cast(expression))


def _ts_or_ds_diff_sql(self: NewBigQuery.Generator, expression: exp.TsOrDsDiff) -> str:
...
def _unix_to_time_sql(self: NewBigQuery.Generator, expression: exp.UnixToTime) -> str:
...
def _str_to_datetime_sql(
    self: NewBigQuery.Generator, expression: exp.StrToDate | exp.StrToTime
) -> str:
...
def _levenshtein_sql(self: NewBigQuery.Generator, expression: exp.Levenshtein) -> str:
...
def _json_extract_sql(self: NewBigQuery.Generator, expression: JSON_EXTRACT_TYPE) -> str:
    ...
    class Parser(parser.Parser):
        ...
        FUNCTIONS = {
            **parser.Parser.FUNCTIONS,
            ...
            "PARSE_DATE": lambda args: build_formatted_time(exp.StrToDate, "newbigquery")(
                [seq_get(args, 1), seq_get(args, 0)]
            ),
        ...
        def _parse_load(self):
            # d = self._match_text_seq("DATA")
            # # i = self._match_text_seq("INTO")
            # i = self._match(TokenType.INTO)
            # _t = self._parse_table(schema=True)
